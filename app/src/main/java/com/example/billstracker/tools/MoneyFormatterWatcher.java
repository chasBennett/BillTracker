...
public class MoneyFormatterWatcher implements TextWatcher {
    ...
    boolean start = formatted.charAt(0) == Objects.requireNonNull(NumberFormat.getCurrencyInstance().getCurrency()).getSymbol().charAt(0);
    ...
    public void beforeTextChanged(CharSequence s, int start1, int count, int after) {
        start = edit.getText().toString().charAt(0) == Objects.requireNonNull(NumberFormat.getCurrencyInstance().getCurrency()).getSymbol().charAt(0);
        edit.removeTextChangedListener(this);
        if (edit.getText().toString().isEmpty() || edit.getText().length() < 4) {
            edit.setText(formatted);
        }
